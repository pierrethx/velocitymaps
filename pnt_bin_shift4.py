...
def get_line4959(wk0, flux, err, wflag):
    """
    Wavelength is rest frame.
    Flux is observed frame - needs to be fixed OR fit in observed frame.
    """
...
def get_line3727(wk0, flux, err, wflag, flag_LDL):
    """
    Wavelength is rest frame for the higher J line.
    Flux is observed frame - needs to be fixed OR fit in observed frame.
...
#j1238
#bin_file_list.append('/Volumes/TOSHIBA EXT/MARTINLAB/'+binning+'z_image.j1238+1009_main_icubes3727_assigned.fits')
...
dest_list.append("/Volumes/TOSHIBA EXT/MARTINLAB/j1238-1/"+binning)
'''
#j0823
...
for fileind in range(len(bin_file_list)):
    ...
    smallbin=np.zeros((int(len(bin)/2),int(len(bin[0])/2)))
    q=0
    for j in range(len(smallbin)):
        for i in range(len(smallbin[0])):
            q+=1
            smallbin[j][i]=q
    bin=np.zeros_like(bin)
    ...
    flux5007, snr5007,p5007,e5007,wline5007 = get_line5007(wave / (1 + zspec), np.sum(allflam_spec,axis=1), np.sum(allflam_espec,axis=1), wflag)
    flux4959, snr4959, p4959, e4959, wline4959 = get_line4959(wave / (1 + zspec), np.sum(allflam_spec,axis=1), np.sum(allflam_espec,axis=1), wflag)
    flux3727, snr3727, dr, sndr, p3727, e3727, wline3727 = get_line3727(wave / (1 + zspec), np.sum(allflam_spec,axis=1), np.sum(allflam_espec,axis=1),wflag, flag_LDL)
    ...
    redshifta=np.mean(redshifts)
    redshiftea=np.sqrt(np.sum(redshiftes**2))/len(redshiftes)
    ...
    for m in range(1,nspec):
        ...
        redshifts=np.divide((locs-wlines),wlines)
        redshiftes=np.divide(locerr,wlines)
        redshift=np.mean(redshifts)
        sig=np.mean(sigs)
        redshifte=np.sqrt(np.sum(redshiftes**2))
        sige=np.sqrt(np.sum(sigerr**2))
